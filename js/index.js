...
const currentMonth = new Intl.DateTimeFormat("uk-UA", { month: "long" }).format(
  new Date()
);
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      const response = await fetch(
        "https://striped-universal-xylocarp.glitch.me/api/orders",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        form.innerHTML = `<h2>Дані успішно відправлені</h2>`;
      } else {
        throw new Error(`Помилка при надсиланні запиту: ${response.status}`);
      }
    } catch (error) {
